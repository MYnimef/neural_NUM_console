...
{
    size_t hiddenLayersAmount = 1; //Кол-во скрытых слоев
    size_t neuronNum[] = {input_col, 5, output_col};    //Кол-во нейронов в каждом слое
    ...
    for (size_t epoch = 0; epoch < maxEpoch; epoch++)   //Начало работы самой нейросети 
    {
        ...
        {
            //Forward propagation
            perceptron[0].forwardPropagation(neuronNum[0], input[train]);
            for (size_t i = 1; i < hiddenLayersAmount + 1; i++)
            {
                perceptron[i].forwardPropagation(perceptron[i - 1].layerSize, perceptron[i - 1].layer);
            }
            ...
            {
                perceptron[i - 1].backwardPropagation(perceptron[i].layerSize, perceptron[i].weights, perceptron[i].sigma);
            }
